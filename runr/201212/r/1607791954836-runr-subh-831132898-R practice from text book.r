#Practice file from Text book

#### Table 2.3 

housing.df <- read.csv("E:/IIMR2014/IIMAcademic-2014/IIM Sirmaur/Text book reference/DMBA-R-datasets/WestRoxbury.csv", header = TRUE)  # load data
...
#scale It provides nothing else but a standardization of the data. The values it creates are known under several different names, 
#one of them being z-scores ("Z" because the normal distribution is also known as the "Z distribution").
set.seed(1)
x <- runif(7)
...
# I thought I would contribute by providing a concrete example of the practical use of the scale function. 
#Say you have 3 test scores (Math, Science, and English) that you want to compare. Maybe you may even want to 
#generate a composite score based on each of the 3 tests for each observation. Your data could look as as thus:
#   
student_id <- seq(1,10)
...
xtotal <- model.matrix(~ 0 + REMODEL.Recent, data = housing.df)
